...
(function() {
  ...
  write_user_info = function(response) {
    var followers, hireable, name, user;
    user = response.data;
  ...
  write_repos = function(response) {
    var count, homepage, i, j, key, lang, language, len, len1, ref, repo, repos, tuple_arr, value;
    repos = response.data;
    ...
    return $.getJSON("vendors/github-language-colors/colors.json", function(clr) {
      var item, k, l, len2, n, ref1, results;
      ref1 = tuple_arr.slice(0, 6);
      results = [];
      for (k = 0, len2 = ref1.length; k < len2; k++) {
        item = ref1[k];
        l = item[0];
        n = item[1];
        results.push($("#skills ul#lang-container").append("<li> <div style=\"background-color:" + clr[l] + "; \"> " + parseInt(n / count * 100) + "% </div><span>" + l + "</span></li>"));
      }
      return results;
    });
  };
